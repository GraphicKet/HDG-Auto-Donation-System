...
if($config["database"]["Is Used"]){	 
		$db=mysqli_connect($config["database"]["Host"],$config["database"]["Username"],$config["database"]["Password"],$config["database"]["Database Name"]);
			...
			if($config["database"]["Password"]){$current .= "YES";}else{$current .= "NO";};
...
if ($p->validate_ipn()) {
		...
		*/
			...
			foreach ($config["rcon"]["Servers"] as &$SERVER){
				if($SERVER["Active"]){
					...
					if( $OUTPUT == 'Unable to connect!' || $OUTPUT == '' ) { 
						if($OUTPUT == ""){$OUTPUT = "No response from server (invalid command?).";}
						WriteLog("GEN", $OUTPUT);
		...
		if($config["forum"]["Is Used"] && !$nicetry){
			...
			/*$fdb = new PDO("mysql:host=$forumhost;dbname=$forumdbname;charset=utf8", "$forumuser", "$forumpass");
			*/
			$query = $ForumQuery;
			//"SELECT * FROM ".$forumprefix."users LEFT JOIN ".$forumprefix."userfields ON ".$forumprefix."users.uid = ".$forumprefix."userfields.ufid WHERE ".$forumprefix."userfields.".$steamidfield." = :steamid";
			$query_params = array(
				':steamid' => $steamid
			);
			$promote = $ForumUpdate;
			//"UPDATE ".$forumprefix."users LEFT JOIN ".$forumprefix."userfields ON mybb_users.uid = ".$forumprefix."userfields.ufid SET ".$forumprefix."users.usergroup = :forumrank WHERE ".$forumprefix."userfields.".$steamidfield." = :steamid";
